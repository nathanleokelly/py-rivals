...
def attack(gameInfo, fighter):
  gameInfo['lastMessage'].append(
      f"Fighter {fighter}, attacked with an:"
  )  #Uses fighter variable to decide which fighter attacked. If the attack is big, it will send "Uppercut".
...
def regenerate(gameInfo, fighter):
  gameInfo['lastMessage'].append(
      f"Fighter {fighter}, took a medpack to regenerate health")
  if fighter == "player":
...
def poison(gameInfo, fighter):
  ...
  if fighter == "player":
    ...
    gameInfo["lastMessage"].append(
        str(gameInfo['poisonDamagePlayer']) +
        " damage points added to your attack from now on")
  else:
    gameInfo['poisonDamageRobot'] += 4
    gameInfo['lastMessage'].append(
        "Robot improved the code of his attack system! ")
    gameInfo['lastMessage'].append(
        str(gameInfo['poisonDamageRobot']) +
        " damage points will be added to each the robots attack from now on.")


def bite(gameInfo):
  if not gameInfo['hasBitten']:
    random_number = random.randint(4, 10)
    gameInfo['biteDamage'] = (random_number +
                              gameInfo['poisonDamagePlayer']) * 2
    ...
    gameInfo['lastMessage'].append(
        "You have already used Serpents final strike. Turn wasted!")
